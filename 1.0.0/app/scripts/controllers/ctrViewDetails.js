     ...
     CommonService.getAll('order/'+$routeParams.userId, uriData, function (data) {
       //  alert(data.OrderDetail[0][1]);
        ...
        $("#receiptInformation").html("<div class='alert with-icon'> <strong>收货人信息</strong>"+
         ...
         //商品总额
        $("#Total").html("<div class='content'><div class='ha30 pull-right col-md-12'><span ><strong>总商品金额：</strong></span><span  style='width:100px; text-align:right'>￥68.00</span></div></div>"+
             ...
             "<div class='ha30 pull-right col-md-12'><strong class='ml10'>实付金额：￥</strong><strong class='blue'>"+data.OrderInfo.amount+"</strong>元</div>");
        ...
        for(var s=0;s<data.payment.length;s++){
            if(data.payment[s][0]==paymentStatus){
                $("#payment").html("<div class='alert with-icon'><strong>支付方式 </strong><div class='content'>"+
                    "<div class='ha30'><input name='radio' type='radio'>"+data.payment[s][0]+" <span class='ml20 blue'>"+data.payment[s][2]+"</span></div>");
            }
        }
    })
